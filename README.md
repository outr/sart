...
| `sart-player/`        | General-purpose cross-platform video/audio player library (over `video_player`), authored in Scala. `PlayerController.bindMediaSession` opt-in publishes to the OS media session (via `sart-tv`'s `audio_service` layer): lock-screen / TV "now playing" + remote transport controls drive the player. Published like the core modules; consumed via `sartLibraries`. Extracted from the NaboTV player. |
