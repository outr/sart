...
class DartEmitter(
  ...
  def inspect(using quotes: Quotes)(tastys: List[Tasty[quotes.type]]): Unit =
    ...
    // before any is emitted — emission decisions (is this parent a wire
    // model? does this nested class flatten?) span compilation units.
    for tasty <- tastys do
      e.register(tasty.ast)
    for tasty <- tastys do
      e.run(tasty.ast)
  ...
  def writeOutput(): Unit =
    ...
    // already promoted), so the diagnostics those trip are silenced.
    // `deprecated_member_use` / `must_be_immutable` are silenced because
    // line-by-line ports faithfully mirror whatever (possibly deprecated
    // or mutable-widget) API the original Dart used — the originals carry
    // the same diagnostics. Everything else stays at full strength.
  ...
  private class Pass[Q <: Quotes & Singleton](
     ...
     *  `@JsonTag("...")` if present, else fabric's default — the
     *  capitalized tail of the fully-qualified name (`Outer.Inner` for a
     *  class nested in an object, the bare name at top level), so a model
     *  module shared with a fabric-serialized JVM backend agrees on the
     ...
     *  `QueryFilterAddressFilter` — the enclosing objects' names prepended.
     *  Facade (`@native`) members keep their own names: they're Dart-side
     *  declarations, not something Sart emits.
     */
    private def nestedFlatName(sym: Symbol): String =
      val own = sym.name.stripSuffix("$")
      val owner = sym.maybeOwner
      // Only classes Sart itself emits flatten — stdlib aliases like
      // `Predef.String` live in objects too, and must keep their names.
      if userClasses.contains(sym) && owner.exists && owner.flags.is(Flags.Module)
         && !owner.isPackageDef && !owner.name.endsWith("$package$")
         && userClasses.contains(owner) then
        nestedFlatName(owner) + own
      else own
     ...
     *  before any `run`, so cross-unit decisions don't depend on order.
     */
    def register(tree: Tree): Unit = registerCompanions(tree)
    ...
    /** Every non-facade class/object Sart itself emits — the set a
     *  member's type must belong to (or be stdlib/facade) to be
    ...
    private def registerCompanions(t: Tree): Unit = t match
      ...
      case cd: ClassDef =>
        ...
        // Classes declared inside objects are emitted (flattened) too.
        cd.body.foreach(registerCompanions)
      case _ => ()
    ...
    private def runTopClass(cd: ClassDef): Unit =
        ...
        if hasNative(sym) then
          ...
          // Classes/objects declared INSIDE the object flatten to top-level
          // Dart classes (`Outer.Inner` → `OuterInner`), whatever the
          // object itself became.
          if !(hasNative(sym) || (moduleVal.exists && hasNative(moduleVal))
               || (companion.exists && hasNative(companion))) then
            cd.body.foreach {
              case inner: ClassDef => runTop(inner)
              case _               => ()
            }
     ...
     *  class) instead of emitting a reference to a type that doesn't exist.
     */
    private def isJvmOnlyGiven(sym: Symbol): Boolean =
      (sym.flags.is(Flags.Given) || sym.flags.is(Flags.Implicit))
        && !isRepresentableType(sym.termRef.widen.dealias)

    /** Principle 4 (loud, not silent): name every skipped JVM-only given. */
    private def emitJvmOnlySkips(cd: ClassDef): Unit =
      cd.body.collect { case vd: ValDef if isJvmOnlyGiven(vd.symbol) => vd }
        .foreach { vd =>
    ...
    private def isRepresentableType(tpe: TypeRepr): Boolean =
      ...
      val fq = sym.fullName
      !sym.exists || hasNative(sym) || userClasses.contains(sym)
        || fq.startsWith("scala.") || fq.startsWith("java.lang.")
        || fq.startsWith("sart.") || fq.startsWith("flutter.")
    ...
    private def emitClassDef(cd: ClassDef): Unit =
      ...
      // instantiates it.
      val className = if sym.name.endsWith("$") then dartName(sym) + "$" else dartName(sym)
      ...
      val memberStats = cd.body.collect {
        ...
        // except non-foldable method defaults, which emit as getters so
        // call-site `$default$` references resolve).
        !neededDefaultGetter(stat.symbol) && (
          stat.symbol.flags.is(Flags.ParamAccessor) ||
          stat.symbol.flags.is(Flags.Synthetic) ||
          stat.symbol.flags.is(Flags.FieldAccessor) ||
      ...
      val keyword =
        ...
        // `abstract` so unimplemented members are legal — Scala traits are
        // never instantiated directly, and abstract mixin classes still mix in.
        else if sym.flags.is(Flags.Trait) && ctorParams.isEmpty
                && parents.extendsTpt.isEmpty && parents.withTpts.isEmpty then "abstract mixin class"
      ...
      // JSON codec synthesis — no annotation needed: every case class
      // whose fields are all wire-representable gets fromJson/toJson (so a
      // model module shared with a JVM backend carries no Sart reference),
      // members of a sealed hierarchy also write their `type` tag, and the
      // sealed parent gets the tag-dispatch factory. `@JsonModel` still
      // forces synthesis; `@JsonTag`/`@JsonField` still override names.
     ...
     */
    private val jsonCaseMemo = mutable.Map[Symbol, Boolean]()
    private def isJsonCase(sym: Symbol): Boolean =
      sym.exists && userClasses.contains(sym)
        && sym.flags.is(Flags.Case) && !sym.flags.is(Flags.Module)
        && !hasNative(sym) && !sym.flags.is(Flags.Enum)
        && (hasJsonModel(sym) || jsonCaseMemo.getOrElseUpdate(sym, {
              // Provisionally true while checking, so a recursive model
              // (`case class Node(children: List[Node])`) terminates.
              jsonCaseMemo(sym) = true
     ...
     */
    private def isJsonSealed(sym: Symbol): Boolean =
      sym.exists && userClasses.contains(sym)
        && sym.flags.is(Flags.Sealed) && !sym.flags.is(Flags.Enum)
        && !hasNative(sym)
        && (hasJsonModel(sym) || sym.children.exists(isJsonCase))

    /** Can a field of this type ride on the synthesized codecs? */
    private def isWireType(rawTpe: TypeRepr): Boolean =
      val tpe = rawTpe.dealias
      val sym = tpe.typeSymbol
      def args: List[TypeRepr] = tpe match
        case AppliedType(_, as) => as.collect { case t: TypeRepr => t }
        case _                  => Nil
      sym.fullName match
        case "scala.Int" | "scala.Long" | "scala.Double" | "scala.Float" | "scala.Boolean"
           | "java.lang.String" | "scala.Predef.String" | "sart.dart.Dyn" | "scala.Any"
           | "fabric.Json" => true
        case "scala.Option" | "sart.stdlib.Option"
           | "scala.collection.immutable.List" | "scala.collection.Seq"
           | "scala.collection.immutable.Seq" | "scala.collection.immutable.Set" =>
          args.forall(isWireType)
        case "scala.collection.immutable.Map" | "scala.Predef.Map" =>
          args match
    ...
    private def emitField(vd: ValDef, static: Boolean = false): Unit =
      ...
      val isLate = isMutable && !baseTpe.endsWith("?") && baseTpe != "dynamic" && vd.rhs.exists {
    ...
    private def emitMethod(dd: DefDef, static: Boolean = false): Unit =
      val sym = dd.symbol
      if sym.flags.is(Flags.Synthetic) && !neededDefaultGetter(sym) then return
      ...
      dd.rhs match
        case Some(rawBody) =>
          ...
          shadowedNames = params.map(_.name).toSet
          emitBodyAsStatements(body, returnLast = retType != "void")
          shadowedNames = savedShadowed
          indent -= 1
          line("}")
          blank()
        case None =>
    ...
    private def emitExpr(t: Term): String = t match
      ...
      case id @ Ident(name) =>
        ...
        // missing companion slot. Annotations may sit on the module val or
        // its module class depending on how the reference resolved.
        val sym = id.symbol
        val carrier =
          if sym.exists && hasNative(sym) then sym
      ...
      // fabric's JSON builders (`obj("k" -> str(v))`, `arr(…)`, `str`/`num`/
      // `bool`) → the Dart literals they denote; fabric.Json is `dynamic`.
      case Apply(fn, List(arg)) if isFabricBuilder(fn, "obj") =>
        val items = varargsItems(arg)
        if items.isEmpty then "<String, dynamic>{}"
        else items.flatMap(mapEntry).mkString("{", ", ", "}")
      case Apply(fn, List(arg)) if isFabricBuilder(fn, "arr") =>
        varargsItems(arg).map(emitExpr).mkString("[", ", ", "]")
      case Apply(fn, List(v)) if isFabricBuilder(fn, "str") || isFabricBuilder(fn, "num")
          || isFabricBuilder(fn, "bool") =>
        emitExpr(v)
      ...
      case Apply(Select(lhs, op), List(rhs)) if isBinaryOp(op) =>
        ...
        val lp = if needsParensAsReceiver(lhs) then s"($l)" else l
        val rp = if needsParensAsReceiver(rhs) then s"($r)" else r
        // Scala's Int/Long division IS integer division; Dart's `/` always
        // produces a double — `~/` is the semantic match (Long is a Dart int too).
        val integral = Set("scala.Int", "scala.Long")
        val dartOp =
          if op == "/" && integral.contains(lhs.tpe.typeSymbol.fullName)
             && integral.contains(rhs.tpe.typeSymbol.fullName) then "~/"
          else op
        s"$lp ${dartOp} $rp"
      ...
      // Bare type application on a member ref (`xs.toSet[B]`, `opt.orNull[U]`)
      // — the type argument has no Dart counterpart, so emit the plain
      // member access (which knows which names are Dart methods vs getters).
    ...
    private def isOptionReceiver(t: Term): Boolean =
      val sym = t.tpe.widen.dealias.typeSymbol
      sym.exists && (sym.fullName == "sart.stdlib.Option" || sym.fullName == "scala.Option"
        // `if a then someOpt else noneOpt` types as the union/lub of the
        // branches — widen doesn't collapse it, so check the class name too.
        || t.tpe.widen.dealias.baseClasses.exists(b =>
             b.fullName == "sart.stdlib.Option" || b.fullName == "scala.Option"))
    ...
    /** A call to fabric's package-level JSON builder `name`. */
    private def isFabricBuilder(fn: Term, name: String): Boolean =
      val sym = fn match
        case id: Ident       => id.symbol
        case s: Select       => s.symbol
        case TypeApply(f, _) => f match
          case id: Ident => id.symbol
          case s: Select => s.symbol
          case _         => Symbol.noSymbol
        case _               => Symbol.noSymbol
      sym.exists && sym.name == name && sym.owner.exists
        && sym.owner.fullName.startsWith("fabric.package")
     ...
     */
    private def selectPrefix(qual: Term, memberName: String = ""): String = qual match
      case This(_) =>
        // `this.` is normally elided, but a method parameter shadowing the
        // field (`copyWith(columnName…) { this.columnName }`) needs it.
        if memberName.nonEmpty && shadowedNames.contains(memberName) then "this." else ""
      case _       =>
        val s = emitExpr(qual)
        if needsParensAsReceiver(qual) then s"($s)." else s"$s."

    /** Parameter names of the method currently being emitted — a
     *  `this.member` whose member shares one of these names keeps its
     *  `this.` qualifier (Dart would otherwise resolve to the param).
     */
    private var shadowedNames: Set[String] = Set.empty
    ...
    private def needsParensAsReceiver(t: Term): Boolean = t match
      ...
      // Predef's implicit wrappers (`doubleWrapper(x - 1).round`) are
      // transparent too — the wrapped binary still needs its parens.
      case Apply(fn, List(inner)) if isScalaImplicitWrapper(fn)               => needsParensAsReceiver(inner)
      case Apply(TypeApply(fn, _), List(inner)) if isScalaImplicitWrapper(fn) => needsParensAsReceiver(inner)
      case _                                               => false
    ...
    /** For a `$default$` reference, the (owner, method-name, 1-based index)
     *  it points at — `ScoresView.queryAndShow$default$2` →
     *  (ScoresView module class, "queryAndShow", 2).
     */
    private def defaultRefInfo(t: Term): Option[(Symbol, String, Int)] =
      def parse(sym: Symbol, n: String): Option[(Symbol, String, Int)] =
        val i = n.indexOf("$default$")
        if i < 0 then None
        else n.substring(i + 9).toIntOption.map(idx => (sym.owner, n.substring(0, i), idx))
      t match
        case NamedArg(_, inner)  => defaultRefInfo(inner)
        case Apply(inner, _)     => defaultRefInfo(inner)
        case TypeApply(inner, _) => defaultRefInfo(inner)
        case s: Select if s.symbol.exists => parse(s.symbol, s.name)
        case id: Ident if id.symbol.exists => parse(id.symbol, id.name)
        case _ => None

    /** A `$default$` arg whose value the emitted Dart signature already
     *  carries (named param with a Dart default) — safe to strip at the
     *  call site. Constructor defaults always strip (ctor params carry
     *  them); method defaults strip only when literal-foldable.
     */
    private def isStrippableDefault(t: Term): Boolean =
      defaultRefInfo(t) match
        case None => true
        case Some((owner, base, idx)) =>
          base == "$lessinit$greater" || base == "copy" || {
            // Only a USER method's non-literal default needs the getter
            // reference — facade/stdlib targets carry real Dart defaults.
            val target = owner.methodMember(base).headOption
            !target.exists(m => isUserCallable(m) && literalDefaultFor(m, idx).isEmpty)
          }

    /** True for a synthesised method `$default$` getter — emitted so any
     *  call site Scala lowered through temp vals (`final x$1 =
     *  Foo.bar$default$2; …`) resolves. Expression-position call sites
     *  still strip/inline them; the getters cost nothing when unused.
     *  Constructor defaults stay un-emitted (their params carry the
     *  default in the emitted ctor signature).
     */
    private def neededDefaultGetter(sym: Symbol): Boolean =
      val n = sym.name
      val i = n.indexOf("$default$")
      i >= 0 && {
        val base = n.substring(0, i)
        // `copy` defaults fold into the synthesised `copyWith`; ctor
        // defaults live in the emitted ctor signature.
        base != "$lessinit$greater" && base != "copy"
          && n.substring(i + 9).toIntOption.isDefined
      }

    /** Emit a `$default$` reference getter-style (`ScoresView.queryAndShow$default$2`). */
    private def emitDefaultRef(t: Term): String = t match
      case NamedArg(_, inner)  => emitDefaultRef(inner)
      case Apply(inner, _)     => emitDefaultRef(inner)
      case TypeApply(inner, _) => emitDefaultRef(inner)
      case s: Select           => s"${selectPrefix(s.qualifier)}${s.name}"
            ...
            case dd: DefDef =>
              dd.rhs.map(unwrapT).flatMap {
                ...
                case Apply(TypeApply(Select(Ident("List"), "apply"), _), List(arg)) if varargsItems(arg).isEmpty =>
                  Some("const []")
                case Apply(Select(Ident("List"), "apply"), List(arg)) if varargsItems(arg).isEmpty =>
                  Some("const []")
                case Apply(TypeApply(Select(Ident("Set"), "apply"), _), List(arg)) if varargsItems(arg).isEmpty =>
                  Some("const {}")
                case Apply(TypeApply(Select(Ident("Map"), "apply"), _), List(arg)) if varargsItems(arg).isEmpty =>
                  Some("const {}")
                case Ident("Nil") | Select(_, "Nil") => Some("const []")
                case _ => None
              }
            case _ => None
        }
     ...
     */
    /** Does `name` occur inside a nested function (lambda/def) within
     *  `root`? Used to keep awaited temp vals from being inlined across a
     *  function boundary.
     */
    private def usedInsideClosure(name: String, root: Tree): Boolean =
      var depth = 0
      var found = false
      val trav = new TreeTraverser:
        override def traverseTree(t: Tree)(owner: Symbol): Unit = t match
          case _: DefDef =>
            depth += 1
            traverseTreeChildren(t)(owner)
            depth -= 1
          case Ident(n) if n == name && depth > 0 =>
            found = true
          case _ =>
            traverseTreeChildren(t)(owner)
      trav.traverseTree(root)(Symbol.spliceOwner)
      found
    ...
    private def inlineTempValBlock(stats: List[ValDef], expr: Term): String =
      ...
      // Strippable defaults (ctor defaults, foldable method defaults) get
      // marker-stripped from the arg list; positional method defaults
      // substitute the emitted `$default$` getter reference instead — the
      // Dart signature has no default to fall back on.
      val (strippable, positionalDefaults) = defaultVals.partition(vd => isStrippableDefault(vd.rhs.get))

      var out = emitExpr(expr)
      for vd <- strippable do out = substitute(out, vd.name, marker)
      for vd <- positionalDefaults do out = substitute(out, vd.name, emitDefaultRef(vd.rhs.get))
      ...
      for vd <- realVals.reverse if !dup do
        // An awaited RHS must not cross a function boundary — inlining it
        // into a (non-async) closure moves the await where it can't run.
        val awaitIntoClosure = vd.rhs.exists(containsAwait) && usedInsideClosure(vd.name, expr)
        if occurrences(out, vd.name) > 1 || awaitIntoClosure then dup = true
        else
    ...
    private def emitTypeRef(tpe: TypeRepr): String =
      ...
      sym.fullName match
        ...
        case "sart.dart.Dyn" =>
          return "dynamic"
        // fabric's JSON value type IS `dynamic` on this side — a shared
        // model module's `Json` fields ride as untyped payloads.
        case "fabric.Json" | "fabric.Obj" | "fabric.Arr" | "fabric.Str" | "fabric.Num"
           | "fabric.Bool" | "fabric.Null" =>
          return "dynamic"
