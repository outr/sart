...
| 0.3 | Models + shared core | Workstream B + D: LN's `lib/model/**` + `service.dart` (916 files) replaced by direct Sart compilation of the shared Scala module; wire format byte-identical; `sartCoreVerify` in CI; `GenerateDart.scala` retired. **Status: bypassed for LN** — the codecs (Workstream B) landed and the port consolidates all ~230 models + 114 endpoints by hand in `models.scala`/`service.scala` (wire-verified field-by-field); the shared module + `GenerateDart` retirement remain open. **2026-09-04:** the real `logicalnetwork-api` module now compiles through the port (an `api-shared` subproject reads `../api/src` directly — single source of truth: `ApiAuth`, the six api request models, `LNApiPaths`, all annotation-free), verified live against the server (login + source list); the ~360 app-only DTOs stay hand-consolidated until the server grows a shared module for them. **2026-09-04, later:** the server NOW HAS a `shared` module — 70 wire DTOs (service request/responses, enums, `user/`, `blm/`) moved to `shared/src`, compiled by the server directly (unmanagedSourceDirectories, byte-identical compilation) and standalone against datawrangler-core only (`sbt shared/compile` is the gate; api parity spec green). Still pinned server-side: DTOs whose lightdb model companions carry DB logic (UnifiedEntity, Source, EntityRecord, Organization…) and everything typed by the heavy `RecordDialect` trait — freeing those needs a `RecordDialect` identity split and companion relocation (server design decisions), plus the emitter's wire-primitive mapping for `lightdb.Id`/`Timestamp`/`EmailAddress` before Sart can compile the shared module through. **2026-09-05: landed** — `sartWireMappings` maps foreign types to Dart primitives or emitted equivalents; the port compiles the shared module (60 DTOs, real enumerations, user documents) straight through with zero analyzer errors, and the side-by-side rig reports all 22 states pixel-identical live. **2026-09-06: RecordDialect identity split done** — shared `RecordDialectId` (name + name-based equality) with the server's `RecordDialect` extending it; DTO fields carry the identity, behavior recovered via `RecordDialect.resolve` at 12 boundary sites; wire unchanged (`RW.string` of the name). Unpinned and moved: 40 more files incl. Source, SourceDetail, Organization(+Access/Invitation), UserDetail, UserOverview, SourceListResponse, the ApiKey family, RespondentType — the shared module now holds ~110 DTO files. Still pinned: the EntityRecord/UnifiedEntity cohort (dialect payload records carry `RecordDialect` pointers + tool imports; DB-bearing model companions), ListRequest (customfields). Port swap of the newly shared mirrors (UserDetail, Source…) done via a transparent-accessor bridge (given Conversion erasure) — 33/35 consume the shared module directly, verified pixel-identical live; OrganizationCreateRequest (com.outr UserAccess generic) and WorkflowListRequest (name-clash, different contract) stay port-mirrored |
